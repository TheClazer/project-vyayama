...
//            System.out.println("Textureiew widthxheight: "+mTextureView.getWidth()+ mTextureView.getHeight());
    ...
    public static CameraFragment create(Bundle SavedInstanceState) {
        ...
        System.out.println("CameraFragment class got data "+runtime_var);
        return fragment;
    }
    /**
            ...
            for (String cameraId : mCameraManager.getCameraIdList()) {
                ...
                switch (displayRotation) {
                    ...
                    case Surface.ROTATION_270:
                        ...
                        break;
                    default:
                        System.out.println("Display rotation is invalid: " + displayRotation);
                }
    ...
    private class CameraSession extends android.hardware.camera2.CameraCaptureSession.CaptureCallback {
        ...
        public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull
            ...
            if (mNetworkLoaded == true) {
                ...
                // show the (rotated) camera frame in the overlay
                mFragmentRender.setFrame(mBitmap);

                // Active-Athlete Lock: classify only the LARGEST detected person (the athlete) —
                // robust to overlapping bystanders, and we coach one subject.
                int primary = -1;
                if (coordslist != null && !coordslist.isEmpty()) {
                    float bestArea = -1f;
                    for (int i = 0; i < BBlist.size() && i < coordslist.size(); i++) {
                        RectangleBox bb = BBlist.get(i);
                        float area = Math.abs((bb.bottom - bb.top) * (bb.right - bb.left));
    ...
    /**
     * Rotate 17 keypoints by `rot` degrees about the frame centre so the classifier sees an UPRIGHT
     * body (matching the overlay's display rotation). (0,0) = missing keypoint → preserved.
     */
    private float[][] rotateKeypoints(float[][] c, int w, int h, int rot) {
        float[][] out = new float[c.length][2];
        double rad = Math.toRadians(rot);
        float cosR = (float) Math.cos(rad), sinR = (float) Math.sin(rad);
        float cx = w / 2f, cy = h / 2f;
        for (int i = 0; i < c.length; i++) {
            if (c[i][0] == 0f && c[i][1] == 0f) { out[i][0] = 0f; out[i][1] = 0f; continue; }
            float dx = c[i][0] - cx, dy = c[i][1] - cy;
            out[i][0] = cx + dx * cosR - dy * sinR;
            out[i][1] = cy + dx * sinR + dy * cosR;
        }
        return out;
    }
